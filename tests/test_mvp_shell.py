...
class DecisionMedWebTest(unittest.TestCase):
    ...
    def test_guided_intake_page_is_local_and_does_not_inject_html(self) -> None:
        ...
        self.assertIn("Por que isso importa?".encode(), body)
        ...
        self.assertIn("Justificativa do direcionamento".encode(), body)
        self.assertIn("Como você quer começar?".encode(), body)
        self.assertIn(b'role="radiogroup"', body)
        self.assertIn(b'id="workflow-nav"', body)
        self.assertIn(b'id="review-tabs-anchor"', body)
        self.assertIn("1. Anamnese".encode(), body)
        self.assertIn("2. Síndrome e diferenciais".encode(), body)
        self.assertIn("3. Exame físico".encode(), body)
        self.assertIn("4. Exames complementares".encode(), body)
        self.assertIn("5. Conduta".encode(), body)
        self.assertIn("Como o sistema chegou a este resultado".encode(), body)
        self.assertIn("Controle sintomático inicial".encode(), body)
        self.assertIn("Tratamento pós-exames".encode(), body)
        self.assertIn(b"query.length<2", body)
